...
void setup()
{
	...
	VertexFormat A = VertexFormat(
		glm::vec3(rectangle.origin.x - (rectangle.breadth / 2.0f),
...
void update()
{
	if (is_colliding(circle, rectangle))
		blue = 1.0f;					// If colliding, then change the color of the object
	else
		blue = 0.0f;					// If not colliding, then revert back to the original color
}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key == GLFW_KEY_W && action == GLFW_PRESS)
		circle.origin.y += moverate;
	if (key == GLFW_KEY_A && action == GLFW_PRESS)
		circle.origin.x -= moverate;
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "Circle AABB collision in 2 Dimension", nullptr, nullptr);

	std::cout << "This is a collision test between a Circle and a Axis aligned bounding box in 2D.\n\n\n\n";
	std::cout << "Use \"w, a, s, d\" to control the circle.\nThe circle changes color upon detecting collision";
